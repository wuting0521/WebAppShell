...
- (void)unregisterHandler:(NSString *)handlerName {
...
- (void)_flushMessageQueueWithError:(NSError *__autoreleasing *)error {
    __strong typeof(self.webView) strongWebView = self.webView;
    
    if ([strongWebView isKindOfClass:[UIWebView class]]) {
        ...
        [self callBackWithMessage:messageQueueString withError:error];
        
    } else if ([strongWebView isKindOfClass:[WKWebView class]]) {
        __weak __typeof(self)weakSelf = self;
        [strongWebView evaluateJavaScript:@"WebViewJavascriptBridge._fetchQueue();" completionHandler:^(id result, NSError *e) {
            ...
            [strongSelf callBackWithMessage:msg withError:error];
        }];
    }
}

- (void)callBackWithMessage:(NSString *)messageQueueString withError:(NSError *__autoreleasing *)error
{
        ...
        if (responseId) {
            ...
            if (message[@"handlerName"]) {
                ...
                if (!handler) {
                    ...
                    if (error)
                    {
                        *error = [NSError errorWithDomain:kWebViewJavascriptBridgeErrorDomain
                                                     code:kWebViewJavascriptBridgeError_UnsupportedAPI
                                                 userInfo:@{kWebViewJavascriptBridgeErrorAPIKey:message[@"handlerName"]}];
                    }
                    return;
                }
            } else {
